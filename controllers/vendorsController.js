        ...
        if (req.files.gallery) {
            ...
            req.files.gallery.forEach((img) => {
                req.body.gallery.push(
                    Date.now() + path.extname(img.originalname)
                );
            });
...
exports.updateVendor = AsyncHandler(async (req, res, next) => {
    ...
    if (req.files.gallery) {
        const writePromises = req.files.gallery.map(async (img, index) => {
            await fs.promises.writeFile(req.gallery[index], img.buffer);
            console.log(index);
        });
        ...
        Promise.all(writePromises)
            ...
            .catch((err) => {
                console.error('Error occurred while saving images:', err);
            });
    }
    return res.status(200).json({
        status: 'success',
